...
// Correct functioning of the app relies on CloseAlertMessage and
// ShareAlertMessage differing
static NSString *const CloseAlertMessage =
    @"Changes have been made to the document that will be lost if not saved";
static NSString *const ShareAlertMessage =
    @"Your changes will not be shared unless the document is first saved";

static void flattenOutline(NSMutableArray *titles, NSMutableArray *pages,
...
@implementation MuDocumentController {
    ...
    UIBarButtonItem *nextButton, *prevButton, *cancelButton, *searchButton,
        *outlineButton, *linkButton;
                                 ...
                                 withAction:(SEL)selector
{
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        ...
        UIButton *button = [UIButton
            buttonWithType:iOS7Style ? UIButtonTypeSystem : UIButtonTypeCustom];
        [button setImage:[UIImage imageNamed:resource]
...
- (void)loadView
{
    ...
    fz_try(ctx) outlineRoot = fz_load_outline(ctx, doc);
    fz_catch(ctx) outlineRoot = NULL;
    if (outlineRoot) {
        //  only show the outline button if there is an outline
        outlineButton = [self newResourceBasedButton:@"ic_list"
                                          withAction:@selector(onShowOutline:)];
        fz_drop_outline(ctx, outlineRoot);
    }

    linkButton = [self newResourceBasedButton:@"ic_link"
                                   withAction:@selector(onToggleLinks:)];
    cancelButton = [self newResourceBasedButton:@"ic_cancel"
                                     withAction:@selector(onCancel:)];
    searchButton = [self newResourceBasedButton:@"ic_magnifying_glass"
                                     withAction:@selector(onShowSearch:)];
    prevButton = [self newResourceBasedButton:@"ic_arrow_left"
                                   withAction:@selector(onSearchPrev:)];
    nextButton = [self newResourceBasedButton:@"ic_arrow_right"
                                   withAction:@selector(onSearchNext:)];
    reflowButton = [self newResourceBasedButton:@"ic_reflow"
                                     withAction:@selector(onToggleReflow:)];
    moreButton = [self newResourceBasedButton:@"ic_more"
                                   withAction:@selector(onMore:)];
    annotButton = [self newResourceBasedButton:@"ic_annotation"
                                    withAction:@selector(onAnnot:)];
    shareButton = [self newResourceBasedButton:@"ic_share"
                                    withAction:@selector(onShare:)];
    printButton = [self newResourceBasedButton:@"ic_print"
                                    withAction:@selector(onPrint:)];
    highlightButton = [self newResourceBasedButton:@"ic_highlight"
                                        withAction:@selector(onHighlight:)];
    underlineButton = [self newResourceBasedButton:@"ic_underline"
                                        withAction:@selector(onUnderline:)];
    strikeoutButton = [self newResourceBasedButton:@"ic_strike"
                                        withAction:@selector(onStrikeout:)];
    inkButton = [self newResourceBasedButton:@"ic_pen"
                                  withAction:@selector(onInk:)];
    tickButton = [self newResourceBasedButton:@"ic_check"
                                   withAction:@selector(onTick:)];
    deleteButton = [self newResourceBasedButton:@"ic_trash"
                                     withAction:@selector(onDelete:)];
    searchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(0, 0, 50, 32)];
    backButton = [self newResourceBasedButton:@"ic_arrow_left"
                                   withAction:@selector(onBack:)];
...
- (void)dealloc
{
    ...
    [slider release];
    slider = nil;
    [sliderWrapper release];
    sliderWrapper = nil;
    [reflowButton release];
    reflowButton = nil;
    [backButton release];
    backButton = nil;
    [moreButton release];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    indicator.text = [NSString
        stringWithFormat:@" %d of %d ", current + 1, fz_count_pages(ctx, doc)];
...
- (void)viewWillLayoutSubviews
{
    ...
    // use max_width so we don't clamp the content offset too early during
    // animation
...
- (void)onShowOutline:(id)sender
{
    ...
    if (root) {
        ...
        [outline release];
        if (titles.count)
            outline = [[MuOutlineController alloc] initWithTarget:self
                                                           titles:titles
                                                            pages:pages];
        [titles release];
        [pages release];
...
- (void)showMoreMenu
{
    NSMutableArray *rightbuttons =
        [NSMutableArray arrayWithObjects:printButton, shareButton, nil];
...
- (void)showAnnotationMenu
{
    self.navigationItem.rightBarButtonItems =
        @[ inkButton, strikeoutButton, underlineButton, highlightButton ];
    self.navigationItem.leftBarButtonItem = cancelButton;
...
- (void)onPrint:(id)sender
{
    ...
    if (pic) {
        ...
        pic.printPageRenderer =
            [[[MuPrintPageRenderer alloc] initWithDocRef:docRef] autorelease];
        ...
        if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
            [pic presentFromBarButtonItem:printButton
                                 animated:YES
    ...
    if (idoc && pdf_has_unsaved_changes(ctx, idoc)) {
        UIAlertView *alertView =
            [[UIAlertView alloc] initWithTitle:AlertTitle
                             ...
                             otherButtonTitles:@"Save and Share", nil];
        [alertView show];
        [alertView release];
    } else {
        [self shareDocument];
...
- (void)searchInDirection:(int)dir
{
    ...
    dispatch_async(queue, ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [searchField setEnabled:YES];
            UIAlertView *alert =
                [[UIAlertView alloc] initWithTitle:@"No matches found for:"
                                           message:@(needle)
                                          delegate:nil
...
- (void)onSlide:(id)sender
{
    ...
    if (slider.tracking)
        indicator.text = [NSString
            stringWithFormat:@" %d of %d ", number + 1, fz_count_pages(ctx, doc)];
    else
        [self gotoPage:number animated:NO];
}
...
- (void)onTap:(UITapGestureRecognizer *)sender
{
    ...
    for (UIView<MuPageView> *view in canvas.subviews) {
        ...
        if (CGRectContainsPoint(view.bounds, pp)) {
            ...
            [result
                switchCaseInternal:^(MuTapResultInternalLink *link) {
...
- (void)createPageView:(int)number
{
    ...
    if (!found) {
        UIView<MuPageView> *view =
            reflowMode ? [[MuPageViewReflow alloc]
                             initWithFrame:CGRectMake(number * width, 0,
                                                      width - GAP, height)
                                  document:docRef
                                      page:number]
                       : [[MuPageViewNormal alloc]
                             initWithFrame:CGRectMake(number * width, 0,
                                                      width - GAP, height)
                             dialogCreator:self
                                   updater:self
...
- (void)invokeTextDialog:(NSString *)aString
              okayAction:(void (^)(NSString *))block
{
    MuTextFieldController *tf =
        [[MuTextFieldController alloc] initWithText:aString
                                         okayAction:block];
...
- (void)invokeChoiceDialog:(NSArray *)anArray
                okayAction:(void (^)(NSArray *))block
{
    MuChoiceFieldController *cf =
        [[MuChoiceFieldController alloc] initWithChoices:anArray
                                              okayAction:block];
...
- (void)didRotateFromInterfaceOrientation:(UIInterfaceOrientation)o
{
    ...
    // size (the maximum of the landscape/portrait widths), to avoid clipping
    // during the rotation.
    canvas.contentSize = CGSizeMake(fz_count_pages(ctx, doc) * width, height);
    canvas.contentOffset = CGPointMake(current * width, 0);
}
